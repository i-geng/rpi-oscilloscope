...
nrf_t *nrf_init(nrf_conf_t c, uint32_t rxaddr, unsigned acked_p) {
    ...
    if(!acked_p) {
        ...
        // Enable pipe 1 and pipe 0?
        nrf_put8_chk(n, NRF_EN_RXADDR, 2);

        // Enable retransmission (1 time?) CORRECT THIS
        ...
        nrf_put8_chk(n, NRF_SETUP_RETR, ((((nrf_default_retran_delay-250)/250)<<4))| nrf_default_retran_attempts);
    ...
    nrf_set_addr(n, NRF_TX_ADDR, 0, nrf_default_addr_nbytes); // FIX THIS
    ...
    if(nrf_pipe_is_enabled(n, 0))
        nrf_set_addr(n, NRF_RX_ADDR_P0, 0, nrf_default_addr_nbytes); // same address okay?
    ...
    // uint8_t check = nrf_get8(n, NRF_STATUS);
    // assert(check == 0 );
    ...
    uint8_t config = nrf_get8(n, NRF_CONFIG);
    nrf_put8_chk(n, NRF_CONFIG, config | 2);
    delay_ms(4.5);
    // DELAY
...
int nrf_get_pkts(nrf_t *n) {
    ...
    do {
        int pipeid = nrf_rx_get_pipeid(n);
        assert( pipeid != NRF_PIPEID_EMPTY);

        uint8_t* buf = kmalloc(n->config.nbytes * sizeof(uint8_t));
