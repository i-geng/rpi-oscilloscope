...
void multi_display_init(void) {
  // TODO: need to initialize multiple displays
...
void multi_display_draw_horizontal_line(int16_t x_start, int16_t x_end, int16_t y,
                                  color_t color) {
  ...
  if ((y >= 0) && (y < MULTI_DISPLAY_HEIGHT)) {
    int16_t w = x_end - x_start;
    uint8_t *p_buf = &multi_display_buffer[(y / 8) * MULTI_DISPLAY_WIDTH + x_start];
    uint8_t mask = 1 << (y & 7);
...
void multi_display_draw_vertical_line(int16_t y_start, int16_t y_end, int16_t x,
  ...
  if ((x >= 0) && (x < MULTI_DISPLAY_WIDTH)) { // if x-coordinate is in bounds
